...
class VirtualmeetingSpider(Spider):
    name = 'virtual_meeting'
    page_number = 1  # Initialize page_number

    def __init__(self):
        self.base_url = "https://www.mea.gov.in/virtual-meetings.htm?3/virtual_meetings"
        self.data = []
    ...
    def save_to_csv(self):
        ...
        data.to_csv(os.path.join(folder_path, 'virtual_meetings_output.csv'), index=False)


# Main function to run the scraper
def main():
    process = CrawlerProcess()
    process.crawl(VirtualmeetingSpider)
    process.start()
